...
@Composable fun NewsPage(sources: List<AnnouncementSource>, rows: List<Announcement>, subscribed: List<String>, keywords: List<AnnouncementKeyword>,
    ...
    PageList {
        item { SectionTitle("校園公告"); CategoryTabs(section, listOf("content" to "公告內容", "subscriptions" to "訂閱管理")) { section = it } }
        when (section) {
            "subscriptions" -> {
                ...
                item { SectionTitle("已接收公告"); ToggleRow("只看關鍵字命中", hitsOnly) { hitsOnly = it }; TextButton(settings) { Text("設定關鍵字、通知與自動更新") } }
...
@Composable fun SettingsV3(options: AppOptions, keywords: List<AnnouncementKeyword>, update: UpdateState, save: (AppOptions) -> Unit, setNotify: (Boolean) -> Unit,
    ...
    PageList {
        item { SectionTitle("設定"); CategoryTabs(selectedSection, listOf("interface" to "介面", "announcements" to "公告", "data" to "資料與版本"), selectSection) }
